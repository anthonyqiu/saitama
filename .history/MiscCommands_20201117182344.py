...
class MiscCommands(commands.Cog):
    ...
    async def butterDog(self, ctx):
        emb = discord.Embed(title = "~B U T T E R   D O G~", description = "The dog with the butter https://itk-assets.nyc3.cdn.digitaloceanspaces.com/2020/11/Screen-Shot-2020-11-10-at-1.33.43-PM-1620x911.jpg", color = 0xffffff);
        emb.set_footer(text = config["defaultFooter"]);
        emb.set_author(name = config["name"], icon_url = config["profilePic"]);
        await ctx.send(embed = emb);
